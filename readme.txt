...
1. Клонируем репо
2. docker compose up
3. Раскомментруем строчку 404 во вьюхе и соответствующую функцию создания значений def generate_values(): строчки 386-395
4. Когда запускаем проект из-под докера в сеттингзах указываем DB_HOST=db, а порт вообще не указываем, убираем его.
папку /pgdata удаляем, если она появилась вручную sudo rm -rf pgdata/ (попробовали запустить, не получилось, а папка от
 ...
 придется постгрес systemctl start postgresql (я останвливаю, чтобы не мешала)
#######################################
...
2. Конечно, необходимость комментировать каждый раз def generate_values(): не самое удачное решение, надо додумать, как
обойти этот процесс комментирования.
